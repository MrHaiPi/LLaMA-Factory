...
if __name__ == '__main__':
    ...
    parser.add_argument('--custom_path', type=str, default="/home/xiarui/.cache/modelscope/hub/models/OpenGVLab/InternVL3-1B", help='Path to custom model config and tokenizer')
    parser.add_argument('--hf_path', type=str, default="/home/xiarui/nas/projects/LLaMA-Factory/output/intern_vl3_1b_lora_sft_20251231_gbs32_lr1e-4_ep40_lk32", help='Path to HF-formatted safetensor weights')
    parser.add_argument('--save_path', type=str, default="/home/xiarui/nas/projects/LLaMA-Factory/output/intern_vl3_1b_lora_sft_20251231_gbs32_lr1e-4_ep40_lk32_hf2cust", help='Path to save converted model')
    ...
    # 把save_path当中的chat_template.jinja文件替换为hf_path当中的chat_template.jinja文件
    import shutil
    # --- 文件复制：用 hf_path 的 chat_template.jinja 覆盖 save_path 中的 ---
    src_chat_template = os.path.join(args.hf_path, "chat_template.jinja")
    dst_chat_template = os.path.join(args.save_path, "chat_template.jinja")

    if os.path.isfile(src_chat_template):
        print(f"\n📝 Copying chat_template.jinja from {src_chat_template} → {dst_chat_template}")
        shutil.copy2(src_chat_template, dst_chat_template)  # copy2 preserves metadata
    else:
        print(f"⚠️ Warning: {src_chat_template} not found. Skipping chat_template.jinja copy.")